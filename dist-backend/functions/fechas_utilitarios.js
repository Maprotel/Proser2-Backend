...
*/
//* Dependencies
var moment = require('moment'); //* Cambiar formato de fecha standard a fecha Unix
...
function standardDate(unixDate) {
  var timestamp = moment.unix(unixDate);
...
Date.createFromMysql = function (mysql_string) {
  ...
  if (typeof mysql_string === 'string') {
    t = mysql_string.split(/[- :]/); //cuando t[3], t[4] and t[5] faltan el valor por defecto es zero
...
function nextDay(fecha_inicio) {
  ...
  var inicio_mas_uno = addDays(fecha_inicio, 1);
  var month = pad(inicio_mas_uno.getUTCMonth() + 1, 2); //months from 1-12

  var day = pad(inicio_mas_uno.getUTCDate(), 2);
  var year = inicio_mas_uno.getUTCFullYear();
...
module.exports = {
  ...
  unix_timestamp_to_string: unix_timestamp_to_string,
  textoEnSegundos: textoEnSegundos,
  mysqlTimeStampToDate: mysqlTimeStampToDate,
  string_to_date: string_to_date,
  hora_a_minutos: hora_a_minutos,
  hora_a_segundos: hora_a_segundos,
  date_text: date_text,
  time_text: time_text,
  ...
  hmsToSecondsOnly: hmsToSecondsOnly
};